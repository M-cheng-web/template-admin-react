...
export const FORM_CREATE_DESIGNER = {
  ...
  desc: t(
    'form-create-designer: 一个通过拖拽的方式快速创建表单的设计器组件, 能提高开发者对表单的开发效率, 节省开发者的时间。',
  ),
};
...
export const UPLOAD_COMPO = {
  ...
  desc: t('ant-design-upload: 使用 ant-design 的 upload 组件, 并具有多种列表展示方式。'),
};
...
export const XLSX_PLUGIN = {
  ...
  desc: t(
    'JS-xlsx: 由SheetJS出品的一款非常方便的只需要纯JS即可读取和导出excel的工具库, 功能强大, 支持xlsx、csv、txt等格式。',
  ),
};
...
export const SORTABLE_PLUGIN = {
  ...
  desc: t('ReactSortable: 基于Sortable.js的react组件, 用以实现拖拽功能。'),
};

// React-Rnd plugin
export const REACT_RND_PLUGIN = {
  name: t('React-Rnd拖拽组件'),
  url: 'https://github.com/bokuweb/react-rnd',
  desc: t('React-Rnd: 一款可调整大小和可拖拽的React组件。'),
};

// React-cropper plugin
export const REACT_CROPPER_PLUGIN = {
  name: t('React-Cropper图片裁剪'),
  url: 'https://github.com/react-cropper/react-cropper',
  desc: t(
    'react-cropper: 一个优雅的图片裁剪插件, 可实现图片裁剪、图片生成等功能, 并支持生成png、jpeg、webp等图片格式。',
  ),
};
...
export const IMAGE_COMPOSITION = {
  ...
  desc: t('图片合成: 基于VueDRR拖拽功能, 在其上通过叠加图片、文字等, 实现图片的叠加伪合成功能。'),
};
...
export const IMAGE_COMPRESS = {
  ...
  desc: t('ImageCompress: 纯JS实现对图片的等比压缩和放大的功能, 并能对图片进行下载。'),
};
...
export const COUNTUP_PLUGIN = {
  ...
  desc: t('React-CountUp: 一个无依赖、轻量级的react数字滚动插件, 以更有趣的方式显示数字数据。'),
};
...
export const REACR_TREE_ORG_PLUGIN = {
  name: t('Tree-Org树状组织图'),
  url: 'https://github.com/artdong/react-org-tree',
  desc: t('react-tree-org: 基于 react 的树结构组织图, 可用于公司组织架构展示。'),
};
...
export const WANG_EDITOR_PLUGIN = {
  ...
  desc: t('wangEditor: 基于javascript和css开发的 Web富文本编辑器, 轻量、简洁、易用、开源免费。'),
};
...
export const MARKDOWN_EDITOR_PLUGIN = {
  ...
  desc: t('React-Md-Editor: 是一款基于 react 的 Markdown 编辑器,比较适合博客系统。'),
};
